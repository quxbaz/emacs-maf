;; -*- lexical-binding: t; -*-
;;
;; Refactor sketch for `sketch/calc-push' (see lib.el).
;;
;; ============================================================================
;; WHY THIS REFACTOR
;; ============================================================================
...
;;
;;   1. A `sketch/calc--target' struct describing what BODY operates on.
;;   2. A pure resolver function `sketch/calc--resolve-target' that decides
;;      what kind of target point is on.
;;   3. A `sketch/calc--write-back' helper that handles the four non-equation
;;      replacement shapes (selection, subexpr, entry, home).
;;   4. A `sketch/calc--cursor-state' struct plus `--capture-cursor-state'
;;      and `--restore-cursor' functions that own the point / window
;;      restoration epilogue.
;;   5. A `sketch/calc-push' dispatch FUNCTION (not a macro) that ties the
;;      above together and runs the user's BODY against the target.
;;   6. A `sketch/defcmd' MACRO that is the only call site of sketch/calc-push;
...
;; there is exactly one macro layer (in `sketch/defcmd') dealing with call-
;; site ergonomics. Users never see `sketch/calc-push'.
;;
;; ============================================================================
;; TRADEOFFS
;; ============================================================================
...
;; `sketch/calc-subformula-at-point' (which rebuilds the selection cache; calc
...
;;      `sketch/calc-active-entry-m-dwim' which prefers the selection on the
...
over both sides. When nil, equations are returned as plain entries."
  (cond
   ...
   ;; ------------------------------------------------------------------------
   ;; `sketch/calc-active-selection-p' is t when calc-use-selections is on AND
   ;; some stack element has a non-nil selection field. The entry we pick
   ;; may not be the one point is on -- see `sketch/calc-active-entry-m-dwim'.
   ...
   ;; ------------------------------------------------------------------------
   ;; `sketch/calc-point-is-at-home-p' is true when point is at the . line or
...
;; the COMMAND-SHELL level (in `sketch/defcmd''s expansion). `sketch/calc-push'
...
;;; ===========================================================================
;;
;; `sketch/calc-push' is a plain function -- not a macro. It is called only
;; from expansions of `sketch/defcmd', which generates a lambda for BODY and
;; passes options as keyword arguments. Keeping the dispatch as a
;; function means:
;;
;;   - The logic is testable in isolation: ERT tests can call sketch/calc-push
;;     directly with a controlled body-fn and assert on stack state.
;;   - There is exactly one macro layer in the file (`sketch/defcmd'), and it
...
;; setup. All three are concerns of the COMMAND SHELL (`sketch/defcmd''s
;; expansion). They are peer pre-body reads that produce data
;; (cursor-state, target struct, arg-val); sketch/calc-push consumes the
;; target and arg-val and does only the stack mutation.
;;
;; SIGNATURE
;;
;; (sketch/calc-push TGT ARG-VAL BODY-FN &key PREFIX WRAP SIMP)
;;
;;   TGT       a `sketch/calc--target' from `sketch/calc--resolve-target'.
  ...
  (let* ((keep-args calc-keep-args-flag)
    ...
    (cl-flet ((run-body (expr replacer)
      ...
      (pcase kind
        ...
        ;; that delegates to `sketch/calc--write-back'. The closure captures
...
;;; ===========================================================================
;;
;; `sketch/defcmd' is the only call site of `sketch/calc-push'. It parses
;; the user's source form, builds a lambda from BODY, and emits a
;; no-arg interactive defun:
;;
;;   (sketch/defcmd NAME (BINDING...) [DOCSTRING]
;;     :key1 val1
;;     :key2 val2
;;     ...
;;     BODY...)
...
;;            (tgt (sketch/calc--resolve-target M LINE? EQ-MAP?))
...
;; call, one cursor restore. `sketch/calc-push' takes the resolved target
...
;;     what `sketch/calc-push' invokes it with. Names the user did NOT
...
;;
;;   - The one existing outlier (`sketch/calc-expand', which takes a
;;     prefix arg) cannot go through `sketch/defcmd' as currently shaped.
...
;;   `sketch/defcmd--validate-bindings' checks BINDINGS against the known
;;     set.
;;   `sketch/defcmd--parse-rest' peels the optional docstring, collects
...
;;
;;   (defun sketch/calc-factor ()
;;     "Factor the active selection or whole stack entry at point."
;;     (interactive)
;;     (sketch/calc-replace-expr-dwim (expr replace-expr) ((prefix "fctr"))
;;       (replace-expr (calcFunc-factor expr))))
;;
;; AFTER:
;;
;;   (sketch/defcmd sketch/calc-factor (expr commit)
;;     "Factor the active selection or whole stack entry at point."
;;     :prefix "fctr"
...
;; BEFORE (current lib.el):
;;
;;   (defun sketch/calc-factor-by ()
;;     "Factor the target expression by the top stack item."
;;     (interactive)
;;     (sketch/calc-replace-expr-dwim (expr replace-expr top)
...
;;
;;   (sketch/defcmd sketch/calc-factor-by (expr commit arg)
;;     "Factor the target expression by the top stack item."
;;     :prefix "fctr"
;;     :simp -1
;;     (let* ((factor arg)
...
;;
;; ---------------------------------------------------------------------------
;; Example 3: prefix-arg-taking command -- calls `sketch/calc-push' directly.
;; ---------------------------------------------------------------------------
;;
;; `sketch/defcmd' supports only no-arg interactive commands. The single
;; command that reads a prefix arg (`sketch/calc-expand') therefore writes
;; the resolve + capture + dispatch + restore dance by hand. It is
;; unary (no `arg' binding), so arg-val is nil and the home-case stack
;; level is 1:
;;
;;   (defun sketch/calc-expand (n)
;;     "Expand the active selection, sub-formula at point, or top stack entry.
;;   With numeric prefix N, expand only N levels deep."
;;     (interactive "P")
;;     (let* ((tgt (sketch/calc--resolve-target 1 nil t))
;;            (cs  (sketch/calc--capture-cursor-state)))
;;       (sketch/calc-push tgt nil
...
;;       (sketch/calc--restore-cursor cs (sketch/calc--target-kind tgt))))
;;
;; This four-line dance is exactly what `sketch/defcmd' generates for the
;; no-arg case. If more prefix-arg-taking commands appear, add
;; `:arglist' and `:interactive' KEYWORDS to `sketch/defcmd' so it can
;; generate the shape automatically.


;; (provide 'sketch/calc/lib-refactor-sketch)
