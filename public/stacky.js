...
//     keysLabel: null | 'square',               // what the keys do, in words: one key that stands in for them
...
export function stack(entries = [], { point = HOME, highlight = null, fresh = null, keys = null, keysLabel = null, result = null } = {}) {
  return { entries: [...entries], point, highlight, fresh, keys, keysLabel, result }
}
...
export const setKeys = (s, keys, keysLabel = null) => ({ ...s, keys, keysLabel })
...
export function lines(s, show = SHOW) {
  ...
  if (s.keys) out.push({ keys: true, prefix: '', text: s.keys, label: s.keysLabel ?? null, cursor: null, mark: null, fresh: false })
  if (s.result != null) {
...
export function lineHtml(l) {
  ...
  if (l.keys) {
    // A label, what the keys do in words, stands in for them, as one key.
    const chips = l.label ? `<kbd class="labeled"><span class="label">${escape(l.label)}</span></kbd>` : l.text.split(' ').filter(Boolean).map(k => `<kbd>${escape(k)}</kbd>`).join('')
