#+title: Plan: equation target per-side iteration
...
For context, see [[file:concepts.org][concepts.org]] (target system overview) and
[[file:walkthrough.org][walkthrough.org]] (a concrete command end-to-end).

* Goal

When point is on a stack entry that is a relation (=, !=, <, <=, >, >=) and the
user invokes a maf-defcmd command, the body should run /once per side/ — once
with =:expr = lhs=, once with =:expr = rhs=. The two results are reassembled
...
*Prefer (A).* Reference uses this pattern: per-side =cl-flet ((replace-expr (e) (setq g-lhs e)))=
and =setq g-rhs e=, then a single =calc-pop-push-record-list= after.
...
1. Read this plan + skim [[file:concepts.org][concepts.org]] + [[file:walkthrough.org][walkthrough.org]] to reload the
   model.
2. Open the reference: =~/.emacs.d/my/calc/lib.el= "Branch 5: equation"
   — that's the prior art. Don't re-derive from scratch.
3. Implement in this order:
   a. =maf--resolve-target-equation= — fill out the schema fields.
   b. Schema doc update at the top of =maf-resolve.el=.
   c. =maf-defcmd= macro — add the equation branch in the expansion.
      Use gensyms for the per-side locals.
   d. =maf--commit='s equation branch — push the reassembled val.
   e. Remove the TODO markers (=grep -rn "equation per-side iteration"=).
