...
(maf-bindings-define '(calc native vim) "\\" #'mafcmd-sqrt)
...
(maf-bindings-define '(calc native vim) "W" #'mafcmd-sqr)
;; Shadows calc-stirling's key; the contextual stirling pair
;; (mafcmd-stir1/stir2) cedes it — see the table in maf-cmds.el.
(maf-bindings-define '(calc native vim) "k s" #'mafcmd-complete-square)
;; Shadows calc-double-factorial's key; mafcmd-dfact cedes it — see
;; the table in maf-cmds.el.
(maf-bindings-define '(calc native vim) "k d" #'mafcmd-factor-powers)
(maf-bindings-define '(calc native vim) "l f" #'mafcmd-factor-by)
(maf-bindings-define '(calc native vim) "l F" #'mafcmd-factor-gcd)
(maf-bindings-define '(calc native vim) "l t" #'mafcmd-collect-fractions)
(maf-bindings-define '(calc native vim) "l l" #'mafcmd-float)
(maf-bindings-define '(calc native vim) "l c" #'mafcmd-frac)
;; Numeric evaluation; k k is unbound in calc itself. I k k routes to
;; mafcmd-identify, the closed-form match for a float.
(maf-bindings-define '(calc native vim) "k k" #'mafcmd-evaluate)
(maf-bindings-define '(calc native vim) "l d" #'mafcmd-to-degrees)
(maf-bindings-define '(calc native vim) "l r" #'mafcmd-to-radians)
;; M-o is unbound in calc itself; H M-o runs the mod-180 variant.
(maf-bindings-define '(calc native vim) "M-o" #'mafcmd-mod-360)
;; Reference angle. M-l is unbound in calc itself (it shadows the global
;; downcase-word, which has no place in the stack buffer).
(maf-bindings-define '(calc native vim) "M-l" #'mafcmd-ref-angle)
;; M-s is unbound in calc itself; it shadows the global `search-map'
;; prefix, which calc buffers have no use for.
(maf-bindings-define '(calc native vim) "M-s" #'mafcmd-supplement)
...
(maf-bindings-define '(calc native vim) "M-c" #'mafcmd-complement)
(maf-bindings-define '(calc native vim) "O" #'mafcmd-commute)
...
(maf-bindings-define '(calc native vim) "<" #'mafcmd-decrement)
(maf-bindings-define '(calc native vim) ">" #'mafcmd-increment)
...
(maf-bindings-define '(calc native vim) "N" #'mafcmd-negate)
...
(maf-bindings-define '(calc native vim) "j l" #'maf-commute-left)
(maf-bindings-define '(calc native vim) "j r" #'maf-commute-right)
...
(maf-bindings-define '(calc native vim) "j e" #'maf-jump-equals)
...
(maf-bindings-define '(calc native vim) "j D" #'maf-distribute)
(maf-bindings-define '(calc native vim) "j M" #'maf-merge)
...
(maf-bindings-define '(calc native vim) "X" #'mafcmd-log-exp)
;; A single-key alias for expand, which also keeps its table key a x.
;; Shadows calc-execute-extended-command.
(maf-bindings-define '(calc native vim) "x" #'mafcmd-expand)
...
;; home motion both.
(maf-bindings-define '(calc native vim) "o" #'mafcmd-inv)
...
(maf-bindings-define '(calc native vim) "&" #'maf-toggle-big-language)
...
(maf-bindings-define '(calc native vim) "G" #'maf-go-home)
;; A toggle between pair members is its own inverse, so both directions
;; run the same command.
(maf-bindings-define '(calc native vim) "S-<up>" #'mafcmd-toggle-op)
(maf-bindings-define '(calc native vim) "S-<down>" #'mafcmd-toggle-op)
(maf-bindings-define '(calc native vim) "," #'maf-quick-variable)
...
(maf-bindings-define '(calc native vim) "p" #'maf-browse-variables)
...
(maf-bindings-define '(calc native vim) "r r" #'maf-recall-variable)
(dotimes (i 10)
...
;; `maf-del' above.
(maf-bindings-define '(calc native vim) "C-M-<backspace>" #'maf-roll-to-bottom)
(maf-bindings-define '(calc native vim) "C-M-DEL" #'maf-roll-to-bottom)
(maf-bindings-define '(calc native vim) "C-M-h" #'maf-roll-to-bottom)
...
(maf-bindings-define '(calc native vim) "S-<return>" #'maf-roll-to-top)
...
(maf-bindings-define '(calc native vim) "M-<up>" #'maf-carry-up)
(maf-bindings-define '(calc native vim) "M-<down>" #'maf-carry-down)
...
(maf-bindings-define '(calc native vim) "=" #'mafcmd-equal-to)
(maf-bindings-define '(calc native vim) "e" #'mafcmd-equal-to)
...
(maf-bindings-define '(calc native vim) "M-." #'mafcmd-remove-equal)
...
(maf-bindings-define '(calc native vim) "@" #'maf-toggle-simplify)
...
(maf-bindings-define '(calc native vim) "C-M-k" #'maf-reset)
(maf-bindings-define '(calc native vim) "C-M-l" #'maf-reset-settings)
...
(maf-bindings-define '(calc native vim) "M-i" #'mafcmd-auto-solve)
...
(maf-bindings-define '(calc native vim) "j i" #'mafcmd-isolate)
...
(maf-bindings-define '(calc native vim) "i" #'mafcmd-solve-for)
;;
...
(maf-bindings-define '(calc native vim) "l v" #'mafcmd-inverse-function)
...
(maf-bindings-define '(calc native vim) "a k" #'mafcmd-abs-ineq)
...
(maf-bindings-define '(calc native vim) "M-u" #'mafcmd-unpack)
...
(maf-bindings-define '(calc native vim) "v RET" #'maf-index)
...
(maf-bindings-define '(calc native vim) "C-|" #'mafcmd-bracket)
...
(maf-bindings-define '(calc native vim) "v L" #'mafcmd-flatten)
...
(maf-bindings-define '(calc native vim) "f l" #'mafcmd-cath)
(maf-bindings-define '(calc native vim) "f L" #'mafcmd-unit-cath)
...
(maf-bindings-define '(calc native vim) "M-k" #'mafcmd-coordinate-toggle)
...
(maf-bindings-define '(calc native vim) "M-m" #'maf-beginning-of-entry)
...
;; resolve.
(maf-bindings-define '(calc native vim) "M-f" #'maf-forward-noun)
(maf-bindings-define '(calc native vim) "M-b" #'maf-backward-noun)
...
(maf-bindings-define '(calc native vim) "S-SPC" #'maf-forward-space)
...
(maf-bindings-define '(calc native vim) "C-M-u" #'maf-up-expression)
...
(maf-bindings-define '(calc native vim) "m c" #'maf-list-modules)
