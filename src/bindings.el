...
(maf-bindings-define '(native vim) "\\" #'mafcmd-sqrt)
...
(maf-bindings-define '(native vim) "W" #'mafcmd-sqr)
;; Shadows calc-stirling's key; the contextual stirling pair
;; (mafcmd-stir1/stir2) cedes it — see the table in maf-cmds.el.
(maf-bindings-define '(native vim) "k s" #'mafcmd-complete-square)
;; Shadows calc-double-factorial's key; mafcmd-dfact cedes it — see
;; the table in maf-cmds.el.
(maf-bindings-define '(native vim) "k d" #'mafcmd-factor-powers)
(maf-bindings-define '(native vim) "l f" #'mafcmd-factor-by)
(maf-bindings-define '(native vim) "l F" #'mafcmd-factor-gcd)
(maf-bindings-define '(native vim) "l t" #'mafcmd-collect-fractions)
(maf-bindings-define '(native vim) "l l" #'mafcmd-float)
(maf-bindings-define '(native vim) "l c" #'mafcmd-frac)
;; Numeric evaluation; k k is unbound in calc itself. I k k routes to
;; mafcmd-identify, the closed-form match for a float.
(maf-bindings-define '(native vim) "k k" #'mafcmd-evaluate)
(maf-bindings-define '(native vim) "l d" #'mafcmd-to-degrees)
(maf-bindings-define '(native vim) "l r" #'mafcmd-to-radians)
;; M-o is unbound in calc itself; H M-o runs the mod-180 variant.
(maf-bindings-define '(native vim) "M-o" #'mafcmd-mod-360)
;; Reference angle. M-l is unbound in calc itself (it shadows the global
;; downcase-word, which has no place in the stack buffer).
(maf-bindings-define '(native vim) "M-l" #'mafcmd-ref-angle)
;; M-s is unbound in calc itself; it shadows the global `search-map'
;; prefix, which calc buffers have no use for.
(maf-bindings-define '(native vim) "M-s" #'mafcmd-supplement)
...
(maf-bindings-define '(native vim) "M-c" #'mafcmd-complement)
(maf-bindings-define '(native vim) "O" #'mafcmd-commute)
...
(maf-bindings-define '(native vim) "<" #'mafcmd-decrement)
(maf-bindings-define '(native vim) ">" #'mafcmd-increment)
...
(maf-bindings-define '(native vim) "N" #'mafcmd-negate)
...
(maf-bindings-define '(native vim) "M" #'mafcmd-map-flag)
...
(maf-bindings-define '(native vim) "j l" #'maf-commute-left)
(maf-bindings-define '(native vim) "j r" #'maf-commute-right)
...
(maf-bindings-define '(native vim) "j e" #'maf-jump-equals)
...
(maf-bindings-define '(native vim) "j D" #'maf-distribute)
(maf-bindings-define '(native vim) "j M" #'maf-merge)
...
(maf-bindings-define '(native vim) "X" #'mafcmd-log-exp)
...
;; home motion both.
(maf-bindings-define '(native vim) "o" #'mafcmd-inv)
...
(maf-bindings-define '(native vim) "&" #'maf-toggle-big-language)
...
(maf-bindings-define '(native vim) "G" #'maf-go-home)
;; A toggle between pair members is its own inverse, so both directions
;; run the same command.
(maf-bindings-define '(native vim) "S-<up>" #'mafcmd-toggle-op)
(maf-bindings-define '(native vim) "S-<down>" #'mafcmd-toggle-op)
(maf-bindings-define '(native vim) "," #'maf-quick-variable)
...
(maf-bindings-define '(native vim) "p" #'maf-browse-variables)
...
(maf-bindings-define '(native vim) "r r" #'maf-recall-variable)
(dotimes (i 10)
...
;; `maf-del' above.
(maf-bindings-define '(native vim) "C-M-<backspace>" #'maf-roll-to-bottom)
(maf-bindings-define '(native vim) "C-M-DEL" #'maf-roll-to-bottom)
(maf-bindings-define '(native vim) "C-M-h" #'maf-roll-to-bottom)
...
(maf-bindings-define '(native vim) "S-<return>" #'maf-roll-to-top)
...
(maf-bindings-define '(native vim) "M-<up>" #'maf-carry-up)
(maf-bindings-define '(native vim) "M-<down>" #'maf-carry-down)
...
(maf-bindings-define '(native vim) "=" #'mafcmd-equal-to)
(maf-bindings-define '(native vim) "e" #'mafcmd-equal-to)
...
(maf-bindings-define '(native vim) "M-." #'mafcmd-remove-equal)
...
(maf-bindings-define '(native vim) "@" #'maf-toggle-simplify)
...
(maf-bindings-define '(native vim) "C-M-k" #'maf-reset)
(maf-bindings-define '(native vim) "C-M-l" #'maf-reset-settings)
...
(maf-bindings-define '(native vim) "M-i" #'mafcmd-auto-solve)
...
(maf-bindings-define '(native vim) "j i" #'mafcmd-isolate)
...
(maf-bindings-define '(native vim) "i" #'mafcmd-solve-for)
;;
...
(maf-bindings-define '(native vim) "l v" #'mafcmd-inverse-function)
...
(maf-bindings-define '(native vim) "a k" #'mafcmd-abs-ineq)
...
(maf-bindings-define '(native vim) "M-u" #'mafcmd-unpack)
...
(maf-bindings-define '(native vim) "v RET" #'maf-index)
...
(maf-bindings-define '(native vim) "C-|" #'mafcmd-bracket)
...
(maf-bindings-define '(native vim) "v L" #'mafcmd-flatten)
...
(maf-bindings-define '(native vim) "f l" #'mafcmd-cath)
(maf-bindings-define '(native vim) "f L" #'mafcmd-unit-cath)
...
(maf-bindings-define '(native vim) "M-k" #'mafcmd-coordinate-toggle)
...
(maf-bindings-define '(native vim) "M-m" #'maf-beginning-of-entry)
...
;; resolve.
(maf-bindings-define '(native vim) "M-f" #'maf-forward-noun)
(maf-bindings-define '(native vim) "M-b" #'maf-backward-noun)
...
(maf-bindings-define '(native vim) "S-SPC" #'maf-forward-space)
...
(maf-bindings-define '(native vim) "C-M-u" #'maf-up-expression)
...
(maf-bindings-define '(native vim) "m c" #'maf-list-modules)
