...
`start`, default `land` — examples below use it), never the default
...
emacsclient -s land --eval '(let ((gframe (seq-find (lambda (f) (frame-parameter f (quote window-system))) (frame-list))))
...
emacsclient -s land --eval '(with-current-buffer "*Calculator*"
...
# then kill them
emacsclient -s land --eval '(delq nil (mapcar (lambda (b)
...
emacsclient -s land --eval '(symbol-function (quote maf-mult))'
```
