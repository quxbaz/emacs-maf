...
(maf-step
  ...
  ;; A marked command behind a prefix key is reached too. No maf binding
  ;; sits behind one now that mafcmd-let has moved off C-c C-c, but the
  ;; provisional path is what a prefix needs and outlives the binding that
  ;; prompted it: at C-c the leaf is unknown, so the flags are let through
  ;; and `maf--fancy-prefix-decide' settles them once mafcmd-let resolves.
  (calc-pop (calc-stack-size))
  (calc-push (math-read-expr "2 x"))
  (calc-push (math-read-expr "x = 3"))
  ...
  ;; Another marked command on another function key: S-<up> is
  ;; mafcmd-toggle-op, and its event is a symbol calc's test rejects the
  ;; same way.
  (calc-pop (calc-stack-size))
