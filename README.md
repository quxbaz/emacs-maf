...
- **Three key profiles.** `calc` keeps Calc's layout and swaps in maf's commands. `ergo`, the default, is maf's own layout. `vim` puts it under `h j k l` motions. Your own bindings sit above and are never rewritten.
